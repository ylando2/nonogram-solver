        ...
        (let ([pos+car (fx+ pos (car lst))]
          ...
          (let loop ([i 0])
            (when (<= i space)
              ...
              (let ([empty-pos (last-empty test (fx+ pos i) (fx+ pos+car i))])
                (if (fx> empty-pos -1)
                    (when (not-full-vector-range? test (fx+ pos i) empty-pos)
                      (fill-vector! temp-vec (fx+ pos i) empty-pos EMPTY)
                      (loop (fx+ (fx- empty-pos pos) 1)))
                    (begin
